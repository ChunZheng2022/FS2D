...
def prepare(root):
    ...
    # process train val test individually
    for i in range(len(splits)):
        ...
        for seg_idx, seg_name in enumerate(seg_file_list):
            ...
            # extract all lidar file under this split, under train for example
            for frame_idx, frame_path in enumerate(frame_path_list):
                assert frame_idx < 1000
                # lidar name is generated by following code, train 0xxxxx, val 1xxxxx, test 2xxxxx
...
class Argo2DatasetMeta(DatasetTemplate):
    ...
    def format_results(self,
        ...
        for i in range(num_samples):
            ...
            # seems like this line caused memory explosion
...
if __name__ == '__main__':
    ...
    # seems like treat all lidar in one feather file as a bin
    save_bin = True
    # get lidar name, path and split
    ...
    velodyne_dir = Path(output_dir) / 'testing' / 'velodyne'
    if not velodyne_dir.exists():
        velodyne_dir.mkdir(parents=True, exist_ok=True)
