...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.use_meta = sampler_cfg.get('USE_META', False)
...
class MetaBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        self.root_path = root_path
        self.class_names = class_names
        self.sampler_cfg = sampler_cfg
        self.num_cls = sampler_cfg.META_N
        self.num_shot = sampler_cfg.META_K
        ...
        self.logger = logger
        self.db_infos = {}
        for class_name in sampler_cfg.FEW_NAME:
            self.db_infos[class_name] = []
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            ...
            with open(str(db_info_path), 'rb') as f:
                infos = pickle.load(f)
                [self.db_infos[cur_class].extend(infos[cur_class]) for cur_class in sampler_cfg.FEW_NAME]
    ...
    def add_meta_set_to_dict(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, num_cls, num_shot):
        gt_boxes = data_dict['gt_boxes']
        ...
        gt_shape = sampled_gt_boxes.shape[-1]
        sampled_gt_boxes = sampled_gt_boxes.reshape(num_cls,num_shot,gt_shape)

        sampled_gt_names = sampled_gt_names.reshape(num_cls,num_shot)

        new_shape = (num_cls, num_shot)
        obj_points_list = [obj_points_list[i:i + new_shape[1]] for i in range(0, len(obj_points_list), new_shape[1])]

        for i in range(num_cls):
            for j in range(num_shot):
                data_dict['gt_boxes_meta'+str(i)+str(j)] = sampled_gt_boxes[i][j]
                # data_dict['gt_names_meta'+str(i)+str(j)] = sampled_gt_names[i][j]
                data_dict['points_meta'+str(i)+str(j)] = obj_points_list[i][j]
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                valid_sampled_dict = sampled_dict
                valid_sampled_boxes = sampled_boxes
        ...
        if total_valid_sampled_dict.__len__() > 0:
            ...
            #     data_dict, sampled_gt_boxes, total_valid_sampled_dict, sampled_mv_height, sampled_gt_boxes2d
            # )
            data_dict = self.add_meta_set_to_dict(
                data_dict, sampled_gt_boxes, total_valid_sampled_dict, self.num_cls, self.num_shot
            )
