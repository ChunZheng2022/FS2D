...
class DataProcessor(object):
    ...
    def transform_meta_points_to_voxels_placeholder(self, data_dict=None, config=None):
        # just calculate grid size
        if data_dict is None:
            grid_size = (self.point_cloud_range[3:6] - self.point_cloud_range[0:3]) / np.array(config.VOXEL_SIZE)
            self.grid_size = np.round(grid_size).astype(np.int64)
            self.voxel_size = config.VOXEL_SIZE
            return partial(self.transform_meta_points_to_voxels_placeholder, config=config)
    ...
    def transform_meta_points_to_voxels(self, data_dict=None, config=None):
        meta_n = config.META_N
        meta_k = config.META_K
        ...
        if data_dict is None:
            ...
            # just bind the config, we will create the VoxelGeneratorWrapper later,
            # to avoid pickling issues in multiprocess spawn
            return partial(self.transform_meta_points_to_voxels, config=config)
        ...
        for i in range(meta_n):
            for j in range(meta_k):

                num_cls = str(i)
                num_shot = str(j)

                points = data_dict['points_meta'+num_cls+num_shot]
                points = common_utils.translate_gt_out_range(points, self.point_cloud_range)
                ...
                if not data_dict['use_lead_xyz']:
                    voxels = voxels[..., 3:]  # remove xyz in voxels(N, 3)
                data_dict['voxels_meta'+num_cls+num_shot] = voxels
                data_dict['voxel_coords_meta'+num_cls+num_shot] = coordinates
                data_dict['voxel_num_points_meta'+num_cls+num_shot] = num_points
        return data_dict
