...
from ...models.model_utils.pruning_block import DynamicFocalPruningDownsample, MetaAggregator
...
class VoxelResBackBone8xVoxelNeXtMeta(nn.Module):
    def __init__(self, model_cfg, input_channels, grid_size, **kwargs):
        ...
        if self.training:
            self.meta_aggregator = MetaAggregator(out_channel)
    ...
    def forward(self, batch_dict):
        ...
        # compress to bev and select unique feature
        ...
        # bev_dense = out.dense()
        # bev_dense = bev_dense.cpu().detach().numpy()
        # for i in range(20):
        #     this_bev_dense = bev_dense[1,i,:,:]
        #     from PIL import Image
        #     Im = Image.fromarray(this_bev_dense)
        #     Im.show()
            ...
            for i in range(self.meta_n):
                for j in range(self.meta_k):
                    ...
                    voxel_features, voxel_coords = (
                        batch_dict['voxel_features_meta'+num_cls+num_shot],
                        batch_dict['voxel_coords_meta'+num_cls+num_shot])
                    input_sp_tensor = spconv.SparseConvTensor(
                        features=voxel_features,
                        indices=voxel_coords.int(),
                        spatial_shape=self.sparse_shape,
                    ...
                    x_meta = self.conv_input(input_sp_tensor)

                    x_conv1_meta = self.conv1(x_meta)
                    x_conv2_meta = self.conv2(x_conv1_meta)
                    x_conv3_meta = self.conv3(x_conv2_meta)
                    x_conv4_meta = self.conv4(x_conv3_meta)
                    ...
                    out_meta = self.bev_out(x_conv4_meta)

                    out_meta = self.conv_out(out_meta)
                    out_meta = self.shared_conv(out_meta)

                    out = self.meta_aggregator(out, out_meta)

            # batch_dict.update({
            #     'encoded_spconv_tensor_meta'+num_cls+num_shot: out,
            # })
            # batch_dict.update({
            #     'multi_scale_3d_features_meta'+num_cls+num_shot: {
            #         'x_conv1': x_conv1,
            #         'x_conv2': x_conv2,
            #         'x_conv3': x_conv3,
            #         'x_conv4': x_conv4,
