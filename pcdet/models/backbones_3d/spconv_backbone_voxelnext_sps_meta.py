    ...
    def __init__(self, in_channels, out_channels, kernel_size, indice_key=None, stride=1, padding=0, pruning_ratio=0.5,
        ...
        if conv_type == 'subm':
            ...
            self.conv = DynamicFocalPruningDownsample(in_channels, out_channels, kernel_size, stride=stride, padding=padding, indice_key=indice_key, bias=False, 
                pruning_ratio=pruning_ratio, pred_mode="learnable", pred_kernel_size=3, loss_mode=loss_mode, algo=algo, pruning_mode=downsample_pruning_mode)
        else:
...
class VoxelResBackBone8xVoxelNeXtSPSMeta(nn.Module):
        ...
        if self.training:
            for i in range(self.meta_n):
                for j in range(self.meta_k):
                    ...
                    x_conv1_meta, batch_dict = self.conv1(x_meta, batch_dict)
                    x_conv2_meta, batch_dict = self.conv2(x_conv1_meta, batch_dict)
                    x_conv3_meta, batch_dict = self.conv3(x_conv2_meta, batch_dict)
                    x_conv4_meta, batch_dict = self.conv4(x_conv3_meta, batch_dict)
                    # x_conv5, batch_dict = self.conv5(x_conv4, batch_dict)
                    # x_conv6, batch_dict = self.conv6(x_conv5, batch_dict)
                    #
                    # x_conv5.indices[:, 1:] *= 2
