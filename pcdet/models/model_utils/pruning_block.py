...
class MetaAggregator(spconv.SparseModule):
    """
    when using topk mode, select_ratio should be set to select num, for example 200
    when using 'ratio' mode or 'thre' mod, it should be set to (0,1), for example 0.5
    """
    ...
    def __init__(self,
        ...
        if pred_mode == "learnable":
            ...
            self.pred_conv = spconv.SubMConv2d(
                in_channels,
                1,
                ...
                padding=padding,
                bias=False,
                indice_key="aggre_pred_conv",
                algo=algo
            )
        self.sigmoid = nn.Sigmoid()

    def get_spatial_shape(self, query_voxel, support_voxel):

        indices = query_voxel.indices
        x = indices[:, 1].max()
        y = indices[:, 2].max()
        query_shape = (int(x) + 1, int(y) + 1)

        indices = support_voxel.indices
        x = max(indices[:, 1]) - min(indices[:, 1])
        y = max(indices[:, 2]) - min(indices[:, 2])
        support_shape = (int(x) + 1, int(y) + 1)

        return query_shape, support_shape

    def generate_sp_tensor_by_importance(self, x, importance):
        batch_size = x.batch_size
        voxel_features_im = []
        voxel_indices_im = []

        for b in range(batch_size):
            features_im, indices_im = self.split_im_voxels(x, b, importance, select_mode=self.select_mode,
    ...
    def split_im_voxels(self, x, b, voxel_importance, select_mode="topk", select_ratio=200):
        index = x.indices[:, 0]
        batch_index = index == b
        ...
        if select_mode == "topk":
            _, indices = voxel_importance.view(-1, ).sort(0, True)
            indices_im = indices[:int(select_ratio)]

        elif select_mode == "ratio":
            _, indices = voxel_importance.view(-1, ).sort()
            indices_im = indices[int(voxel_importance.shape[0] * select_ratio):]

        elif select_mode == "thre":
            indices_im = (voxel_importance.view(-1, ) > select_ratio)

        features_im = features_ori[indices_im]
        coords_im = indices_ori[indices_im]

        return features_im, coords_im

    # def agg_conv(query_voxel, support_voxel, query_shape, support_shape, stride=1):
    #     support_feature = support_voxel.features
    #     query_feature = query_voxel.features
    #     batch = query_voxel.batch_size
    #     query_indices = query_voxel.indices
    #     support_sum = torch.sum(support_feature, dim=0)
    #     score_bev = torch.zeros(query_shape[0] * query_shape[1], query_feature.shape[-1]).cuda()
    #     score_indice = torch.zeros(query_shape[0] * query_shape[1], query_indices.shape[-1]).cuda()
    #     score_num = 0
    #     window_stride = (stride, stride) if isinstance(stride, int) else stride
    #     for b in range(batch):
    #         for i in range(0, query_shape[0], window_stride[0]):
    #             for j in range(0, query_shape[1], window_stride[1]):
    #                 col2_mask = torch.logical_and(query_indices[:, 1] >= i, query_indices[:, 1] <= i + support_shape[0])
    #                 col3_mask = torch.logical_and(query_indices[:, 2] >= j, query_indices[:, 2] <= j + support_shape[1])
    #                 combined_mask = torch.logical_and(col2_mask, col3_mask)
    #                 this_indice = torch.tensor([b, i, j])
    #                 window_feature = query_feature[combined_mask]
    #                 if not window_feature.shape[0] == 0:
    #                     window_sum = torch.sum(window_feature, dim=0)
    #                     diff = abs(window_sum - support_sum)
    #                     score_bev[score_num] = 1.0 / (diff + 1e-6)
    #                 else:
    #                     score_bev[score_num] = 1.0 / (support_sum + 1e-6)
    #                 score_indice[score_num] = this_indice
    #                 score_num += 1
    #     return score_bev, score_indice

    def agg_conv(self, im_voxel, query_voxel, support_voxel, query_shape, support_shape):
        support_feature = support_voxel.features
        query_feature = query_voxel.features
        # batch = query_voxel.batch_size
        query_indices = query_voxel.indices
        im_indices = im_voxel.indices
        support_sum = torch.sum(support_feature, dim=0)
        score_bev = []
        score_indice = []
        half_h = support_shape[0] // 2
        half_w = support_shape[1] // 2

        # input_hash = {}
        #
        # for i in range(im_indices.shape[0]):
        #     index_key = tuple(im_indices[i])
        #     input_hash[index_key] = query_feature[i]
        # output_hash = {}
        # index_out = 0
        #
        # for b in range(batch):
        #     for h in range(query_shape[0]):
        #         for w in range(query_shape[1]):
        #             index_key = tuple([b, h, w])
        #             output_hash[index_key] = index_out
        #             index_out += 1

        for idx, indice in enumerate(im_indices):
            q_idx_b, q_idx_h, q_idx_w = indice

            mask_col0 = query_indices[:, 0] == q_idx_b

            # calculate window bound
            start_h = q_idx_h - half_h
            end_h = q_idx_h + half_h
            start_w = q_idx_w - half_w
            end_w = q_idx_w + half_w

            is_center = (start_h >= 0) and (start_w >= 0) and (end_h <= query_shape[0]) and (end_w <= query_shape[1])

            window_feature = []

            if is_center:
                mask_col1 = torch.logical_and(query_indices[:, 1] > q_idx_h - half_h,
                                              query_indices[:, 1] < q_idx_h + half_h)
                mask_col2 = torch.logical_and(query_indices[:, 2] > q_idx_w - half_w,
                                              query_indices[:, 2] < q_idx_w + half_w)
                combined_mask1 = torch.logical_and(mask_col0, mask_col1)
                combined_mask = torch.logical_and(combined_mask1, mask_col2)
                window_feature = query_feature[combined_mask]
                # for i in range(-half_h, half_h):
                #     for j in range(-half_w, half_w):
                #         index_key = tuple([q_idx_b, q_idx_h + i, q_idx_w + j])
                #         if index_key in input_hash:
                #             window_feature.append(input_hash[index_key])
                if len(window_feature) > 0:
                    window_sum = torch.sum(window_feature, dim=0)
                    diff = torch.abs(window_sum - support_sum)
                    score = 1.0 / (diff + 1e-6)
                    score_bev.append(score)
                else:
                    score = 1.0 / (support_sum + 1e-6)
                    score_bev.append(score)
                score_indice.append(indice)

            else:
                if ((start_h < 0) or (end_h > query_shape[0])) and ((start_w < 0) or (end_w > query_shape[0])):
                    if start_h < 0 and start_w < 0:
                        offset_h = 0 - start_h
                        offset_w = 0 - start_w
                        mask_col1 = torch.logical_and(query_indices[:, 1] > q_idx_h - half_h + offset_h,
                                                      query_indices[:, 1] < q_idx_h + half_h + offset_h)
                        mask_col2 = torch.logical_and(query_indices[:, 2] > q_idx_w - half_w + offset_w,
                                                      query_indices[:, 2] < q_idx_w + half_w + offset_w)
                        combined_mask1 = torch.logical_and(mask_col0, mask_col1)
                        combined_mask = torch.logical_and(combined_mask1, mask_col2)
                        window_feature = query_feature[combined_mask]
                        # for i in range(-half_h + offset_h, half_h + offset_h):
                        #     for j in range(-half_w + offset_w, half_w + offset_w):
                        #         index_key = tuple([q_idx_b, q_idx_h + i, q_idx_w + j])
                        #         if index_key in input_hash:
                        #             window_feature.append(input_hash[index_key])
                    elif start_h < 0 and end_w > query_shape[0]:
                        offset_h = 0 - start_h
                        offset_w = end_w - query_shape[0]
                        mask_col1 = torch.logical_and(query_indices[:, 1] > q_idx_h - half_h + offset_h,
                                                      query_indices[:, 1] < q_idx_h + half_h + offset_h)
                        mask_col2 = torch.logical_and(query_indices[:, 2] > q_idx_w - half_w - offset_w,
                                                      query_indices[:, 2] < q_idx_w + half_w - offset_w)
                        ...
                        #         if index_key in input_hash:
                        #             window_feature.append(input_hash[index_key])
                    elif end_h <= query_shape[0] < 0 and end_w > query_shape[0]:
                        offset_h = 0 - start_h
                        offset_w = end_w - query_shape[0]
                        mask_col1 = torch.logical_and(query_indices[:, 1] > q_idx_h - half_h - offset_h,
                                                      query_indices[:, 1] < q_idx_h + half_h - offset_h)
                        ...
                        #         if index_key in input_hash:
                        #             window_feature.append(input_hash[index_key])
                    elif end_h <= query_shape[0] < 0 and start_w < 0:
                        offset_h = 0 - start_h
                        offset_w = 0 - start_w
                        mask_col1 = torch.logical_and(query_indices[:, 1] > q_idx_h - half_h - offset_h,
                                                      query_indices[:, 1] < q_idx_h + half_h - offset_h)
                ...
                elif (start_h < 0) or (end_h > query_shape[0]):
                    if start_h < 0:
                        offset = 0 - start_h

                        mask_col1 = torch.logical_and(query_indices[:, 1] > q_idx_h - half_h + offset,
                                                      query_indices[:, 1] < q_idx_h + half_h + offset)
                        mask_col2 = torch.logical_and(query_indices[:, 2] > q_idx_w - half_w,
                                                      query_indices[:, 2] < q_idx_w + half_w)
                        combined_mask1 = torch.logical_and(mask_col0, mask_col1)
                        combined_mask = torch.logical_and(combined_mask1, mask_col2)
                        ...
                        # for i in range(-half_h + offset, half_h + offset):
                        #     for j in range(-half_w, half_w):
                        #         index_key = tuple([q_idx_b, q_idx_h + i, q_idx_w + j])
                        #         if index_key in input_hash:
                        #             window_feature.append(input_hash[index_key])
                    else:
                        offset = end_h - query_shape[0]
                    ...
                    if start_w < 0:
                        ...
                        mask_col1 = torch.logical_and(query_indices[:, 1] > q_idx_h - half_h,
                                                      query_indices[:, 1] < q_idx_h + half_h)
                        mask_col2 = torch.logical_and(query_indices[:, 2] > q_idx_w - half_w + offset,
                                                      query_indices[:, 2] < q_idx_w + half_w + offset)
                        combined_mask1 = torch.logical_and(mask_col0, mask_col1)
                        combined_mask = torch.logical_and(combined_mask1, mask_col2)
                        window_feature = query_feature[combined_mask]

                        # for i in range(-half_h, half_h):
                        #     for j in range(-half_w + offset, half_w + offset):
                        #         index_key = tuple([q_idx_b, q_idx_h + i, q_idx_w + j])
                        #         if index_key in input_hash:
                        #             window_feature.append(input_hash[index_key])
                    else:
                        offset = end_w - query_shape[0]
        ...
        score_bev = torch.stack(score_bev)
        score_bev = self.sigmoid(score_bev)
        score_indice = torch.stack(score_indice)

        return score_bev, score_indice

    # def aggregate_with_score(score_bev, score_indice, query_voxel):
    #     query_feature = query_voxel.features
    #     query_indices = query_voxel.indices
    #
    #     for i in range(query_indices.shape[0]):
    #         for j in range(score_indice.shape[0]):
    #             if (query_indices[i] == score_indice[j]).all():
    #                 query_feature[i] += score_bev[j]
    #
    #     query_voxel.replace_feature(query_feature)
    #     return query_voxel

    def aggregate_with_score(self, score_bev, score_indice, query_voxel):
        query_feature = query_voxel.features.detach().cpu().numpy()
        query_indices = query_voxel.indices.detach().cpu().numpy()

        score_bev = score_bev.detach().cpu().numpy()
        score_indice_cpu = score_indice.detach().cpu().numpy()

        score_hash = {}
        for j in range(score_indice_cpu.shape[0]):
            index_key = tuple(score_indice_cpu[j])
            score_hash[index_key] = score_bev[j]

        for i in range(query_indices.shape[0]):
            index_key = tuple(query_indices[i])
            if index_key in score_hash:
                query_feature[i] += score_hash[index_key]

        query_feature = torch.tensor(query_feature).cuda()
        query_voxel.replace_feature(query_feature)
        return query_voxel

    def aggregate_meta_feature(self, query_voxel, support_voxel, voxel_importance):

        query_shape, support_shape = self.get_spatial_shape(query_voxel, support_voxel)

        important_voxel = self.generate_sp_tensor_by_importance(query_voxel, voxel_importance)

        score_bev, score_indice = self.agg_conv(important_voxel, query_voxel, support_voxel, query_shape, support_shape)

        aggregated_voxel = self.aggregate_with_score(score_bev, score_indice, query_voxel)

        return aggregated_voxel

    def forward(self, input_voxel, input_meta):
        ...
        if self.pred_mode == "learnable":
            ...
            x_features = input_voxel.features
        ...
        x = self.aggregate_meta_feature(input_voxel, input_meta, voxel_importance)
...
class DynamicFocalPruningConv(spconv.SparseModule):
    def __init__(self,
        ...
        if mask_caculate_mode == "avg_pool":
            ...
            channel_mask = torch.eye(max(channel_idx) + 1)[channel_idx].view(1, 1, 1, out_channels,
                                                                             out_channels)  # convert 2 onhot
    ...
    def visualize(self, x, x_attn_predict):
        ...
        print("max_v value:", max_v, "min_v value:", min_v, "norm_dense_map min:", norm_dense_map.min(),
              "norm_dense_map max:", norm_dense_map.max(), "norm_dense_map mean:", norm_dense_map.mean())
        ...
        plt.imshow(color)
        plt.savefig(
            "/data/home/jianhuiliu/newData/Research/CVMI-3D-Pruning/3d-detect-pruning-develop/output/visual/plot_" + self.indice_key + "_" + str(
                x.features.shape[0]) + "_.png")
...
class DynamicFocalPruningDownsample(spconv.SparseModule):
    ...
    def generate_sparse_tensor(self, x, voxel_importance):
        batch_size = x.batch_size
        voxel_features_im = []
        voxel_indices_im = []
        voxel_features_nim = []
    ...
    def forward(self, x, batch_dict):
        ...
        x_im, x_nim = self.generate_sparse_tensor(x, voxel_importance)
